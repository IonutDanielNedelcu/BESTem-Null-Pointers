...
# 1. Setup: încarcă tokenizerul și modelul doar o singură dată
checkpoint_path = "D:/University/BEST/BEST/bert_fight_classifier"
device = torch.device("cpu")  # Folosește doar CPU

tokenizer = AutoTokenizer.from_pretrained(checkpoint_path)
model = AutoModelForSequenceClassification.from_pretrained(checkpoint_path)
model.eval()
model.to(device)  # Mutăm modelul pe CPU
...
# 3. Funcție predict fără măsurarea timpului
@torch.no_grad()
def predict(cuvant: str) -> int:
    # Tokenizare și predicție
    inputs = tokenizer(cuvant, padding="max_length", truncation=True, max_length=32, return_tensors="pt").to(device)
    logits = model(**inputs).logits.squeeze()

    # Obține top 3 predicții
    probs = torch.softmax(logits, dim=-1)
    top3_probs, top3_indices = probs.topk(3)

    # Căutăm cel mai mic cost
    best_score = 101
    best_id = 0
